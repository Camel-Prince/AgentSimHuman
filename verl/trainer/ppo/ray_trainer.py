...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
        ...
        # Use per-segment credit assignment when segment_ids are available
        if 'segment_ids' in data.batch:
            segment_ids = data.batch['segment_ids'][:, -response_length:]
            num_rounds = data.meta_info.get('num_revision_rounds', 3)
            advantages, returns = core_algos.compute_grpo_paper_writing_advantage(
...
class RayPPOTrainer(object):
    ...
    def _is_paper_writing_task(self) -> bool:
    ...
    def _run_generation_by_task_type(self, generation_manager: LLMGenerationManager, gen_batch: DataProto,
        ...
        task_type = str(getattr(self.config, 'task_type', '') or '')
        if task_type == 'paper_writing':
            return generation_manager.run_llm_loop_paper_writing(
                gen_batch=gen_batch,
                initial_input_ids=first_input_ids,
            )
        if task_type == 'paper_writing_last_round_target':
            return generation_manager.run_llm_loop_paper_writing_last_round_target(
                gen_batch=gen_batch,
                initial_input_ids=first_input_ids,
            )
        if task_type == 'paper_writing_per_segment':
            return generation_manager.run_llm_loop_paper_writing_per_segment(
                gen_batch=gen_batch,
                initial_input_ids=first_input_ids,
            )
        if task_type == 'paper_writing_train_commenter':
            return generation_manager.run_llm_loop_paper_writing_train_commenter(
                gen_batch=gen_batch,
                initial_input_ids=first_input_ids,
            )
        if task_type == 'paper_writing_arena_seeded':
            return generation_manager.run_llm_loop_paper_writing_arena_seeded(
                gen_batch=gen_batch,
                initial_input_ids=first_input_ids,
            )
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            num_revision_rounds=self.config.get('num_revision_rounds', 3),
            commenter_api_key=self.config.get('commenter_api_key', None),
            commenter_base_url=self.config.get('commenter_base_url', None),
            commenter_model=self.config.get('commenter_model', 'qwen-plus'),
            generator_max_concurrency=self.config.get('generator_max_concurrency', 64),
            commenter_max_concurrency=self.config.get('commenter_max_concurrency', 48),
            api_rpm=self.config.get('api_rpm', 120),
        ...
        generation_manager = LLMGenerationManager(
            ...
            ref_policy_wg=self.ref_policy_wg if self.use_reference_policy else None,
            config=gen_config,
            is_validation = True,
        )
        ...
        if not self.config.do_search:
            ...
            for batch_dict in self.val_dataloader:
                ...
                test_gen_batch = test_batch.pop(batch_keys=['input_ids', 'attention_mask', 'position_ids'],
                                                 non_tensor_batch_keys=['extra_info', 'reward_model'] if self._is_paper_writing_task() else [])
                test_gen_batch.meta_info = {
                    'eos_token_id': self.tokenizer.eos_token_id,
                    'pad_token_id': self.tokenizer.pad_token_id,
                    'recompute_log_prob': False,
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('gen', timing_raw):
                        ...
                        final_gen_batch_output = self._run_generation_by_task_type(
    ...
    def _save_checkpoint(self):
        ...
        actor_local_path = os.path.join(self.config.trainer.default_local_dir, 'actor',
                                        step_tag)
        # Ensure local checkpoint directories exist before worker-side save.
        os.makedirs(actor_local_path, exist_ok=True)
        actor_remote_path = None if self.config.trainer.default_hdfs_dir is None else os.path.join(
            self.config.trainer.default_hdfs_dir, 'actor', step_tag)
        self.actor_rollout_wg.save_checkpoint(actor_local_path, actor_remote_path)

        if self.use_critic:
            critic_local_path = os.path.join(self.config.trainer.default_local_dir, 'critic',
                                             step_tag)
            os.makedirs(critic_local_path, exist_ok=True)
            critic_remote_path = None if self.config.trainer.default_hdfs_dir is None else os.path.join(
                self.config.trainer.default_hdfs_dir, 'critic', step_tag)
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    # with _timer('step', timing_raw):
                        ...
                        with torch.no_grad():
                            # final_gen_batch_output.meta_info.update({
                            #     'micro_batch_size': self.config.actor_rollout_ref.rollout.log_prob_micro_batch_size,
                            #     'temperature': self.config.actor_rollout_ref.rollout.temperature,
                            #     'use_dynamic_bsz': self.config.actor_rollout_ref.rollout.log_prob_use_dynamic_bsz,
                            #     'max_token_len': self.config.actor_rollout_ref.rollout.log_prob_max_token_len_per_gpu,
                            ...
                            #     'pad_token_id': self.tokenizer.pad_token_id 
                            # }) 
                            '''不能再手动更新啦，compute_log_prob(final_gen_batch_output) 要吃的是 worker 视角的本地可执行参
                            数，而这正是 FSDP 归一化的目的；
                            比如：全局的log_pro_micro_batch_size即gen_batch.meta_info['micro_batch_size']是4，
                            fsdp自动归一化后即final_gen_batch_output.meta_info['micro_batch_size']是1，
                            那么compute_log_prob函数接受的micro_batch_size应该是1'''
                            if 'old_log_probs' not in final_gen_batch_output.batch:
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # collect detailed paper-writing reward metrics for wandb
                    ...
                    # implement critic warmup; only update actor after critic warmup

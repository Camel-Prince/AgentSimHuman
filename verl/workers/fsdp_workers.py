    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        if self.rank == 0:
            ...
            if hdfs_path is not None:
                dst_path = os.path.expanduser(hdfs_path)
                print(f'Uploading actor checkpoint to {dst_path}')
                if dst_path.startswith("hdfs://"):
                    hdfs_io.makedirs(dst_path, exist_ok=True)
                    hdfs_io.copy(src=local_path, dst=dst_path)
                else:
                    parent_dir = os.path.dirname(dst_path)
                    if parent_dir:
                        os.makedirs(parent_dir, exist_ok=True)
                    # Local copytree requires dirs_exist_ok=True for resumed/retried saves.
                    hdfs_io.copy(src=local_path, dst=dst_path, dirs_exist_ok=True)
...
class CriticWorker(Worker):
    ...
    def save_checkpoint(self, local_path, hdfs_path=None):
        ...
        if self.rank == 0:
            ...
            if hdfs_path is not None:
                dst_path = os.path.expanduser(hdfs_path)
                print(f'Uploading critic checkpoint to {dst_path}')
                if dst_path.startswith("hdfs://"):
                    hdfs_io.makedirs(dst_path, exist_ok=True)
                    hdfs_io.copy(src=local_path, dst=dst_path)
                else:
